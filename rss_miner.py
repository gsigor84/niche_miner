...
def parse_keywords(raw_keywords: str, prefix: Optional[str] = None) -> List[str]:
    """Parse comma-separated CLI keywords, applying the same prefix behavior as seed files."""
    keywords: List[str] = []
    for raw in raw_keywords.split(","):
        keyword = raw.strip()
        if not keyword:
            continue
        if prefix and not keyword.lower().startswith(prefix.lower()):
