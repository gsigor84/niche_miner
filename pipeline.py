...
def parse_scout_subreddits(output):
    """Extract a comma-separated subreddit list from scout_subreddits output."""
    match = re.search(r"--subs\s+([A-Za-z0-9_,\-]+)", output or "")
    if not match:
        return None
    subs = [s.strip() for s in match.group(1).split(",") if s.strip()]
...
def parse_args():
    ...
    p.add_argument("--max_keywords", type=int, default=20, help="Max keywords to use for Reddit search")
    p.add_argument("--only_pain_points", action="store_true", help="Only fetch pain-point-ish Reddit posts")
    # gap phase
...
def main():
    ...
    # fresh runs do not skip downstream work based on stale initial state.
    if args.keywords:
