...
class SeedFactory:
    ...
    def save(self):
        ...
        if not sorted_seeds:
            print("[ERROR] No seeds generated; refusing to overwrite seed_topics.txt with an empty file.")
            return False
...
def main():
    ...
    if args.source == "google":
        source_ok = factory.harvest_google_taxonomy()
    elif args.source == "llm":
        if not args.topic:
            print("[ERROR] --topic is required for LLM source.")
            return 2
        source_ok = factory.brainstorm_llm(args.topic, count=args.count, model=args.model)
    
    if not source_ok:
        print("[ERROR] Seed generation failed; existing seed_topics.txt was left unchanged.")
        return 1
