...
def parse_scout_subreddits(output):
    """Extract the machine-usable --subs list from scout_subreddits output."""
    for line in output.splitlines():
        line = line.strip()
        marker = "To use with rss_miner: --subs "
        if line.startswith(marker):
            return [s.strip() for s in line[len(marker):].split(",") if s.strip()]
    return []
...
def parse_args():
    ...
    p.add_argument("--topic", "--niche", dest="topic", default=None,
                   help="Topic for seed_factory (LLM brainstorming)")
    ...
    p.add_argument("--only_pain_points", action="store_true",
                   help="Only save pain-point-ish posts during fetch")
    # gap phase
...
def main():
    ...
    # Sequential pipeline. Re-check state after each phase so a fresh run can
    # progress past seed/scout in the same invocation.
    ran_any_phase = False
    ...
    for phase_name in PHASES:
        if phase_name == "seed" and (args.skip_seed or state.get("seed") == "done"):
            continue
        if phase_name == "scout" and (args.skip_scout or state.get("scout") == "done"):
            continue
        if phase_name == "fetch" and state.get("fetch") == "done":
            continue
        if phase_name == "normalize" and state.get("normalize") == "done":
            continue
        if phase_name == "gap" and (args.skip_gap or state.get("gap") == "done"):
            continue

        if phase_name == "fetch" and not (
            state.get("scout") == "done" or state.get("seed") == "done"
        ):
            continue
        ...
        print(f"{'#'*60}")
        phase_fns[phase_name](args, args.run_id, state)
        ran_any_phase = True
        time.sleep(0.5)
