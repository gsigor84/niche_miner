...
def main():
    ...
    # Sequential pipeline. Decide each phase immediately before running it so
    # phases completed earlier in this invocation unlock downstream work.
    if args.keywords:
    ...
    for phase_name, phase_fn in phase_plan:
        if phase_name == "seed":
            if args.skip_seed or state.get("seed") == "done":
                continue
        elif phase_name == "scout":
            if args.skip_scout or state.get("scout") == "done":
                continue
        elif phase_name == "fetch":
            if state.get("fetch") == "done":
                continue
            if state.get("seed") != "done" and not args.skip_seed:
                continue
        elif phase_name == "normalize":
            if state.get("normalize") == "done" or state.get("fetch") != "done":
                continue
        elif phase_name == "gap":
            if args.skip_gap or state.get("gap") == "done" or state.get("normalize") != "done":
                continue
