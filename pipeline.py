...
def run_phase_scout(args, run_id, state):
    ...
    subs_output = run_path(run_id, "subreddits.txt")
    cmd = [
        "python3", "scout_subreddits.py", keywords,
        "--limit", str(args.max_seeds),
        "--output", str(subs_output),
    ]
    print(f"\n[PHASE 2] Keywords: {keywords[:100]}...")
    run(cmd, "PHASE 2: scout_subreddits")
    if not subs_output.exists() or not subs_output.read_text(encoding="utf-8").strip():
        print(f"[ERROR] Scout produced no subreddit output: {subs_output}")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    scouted_subs = run_path(run_id, "subreddits.txt")
    subs = args.subs
    if not subs and scouted_subs.exists():
        subs = scouted_subs.read_text(encoding="utf-8").strip()
    if not subs:
        subs = args.niche_type
...
def main():
    ...
    skipped = set()
    if args.skip_seed or args.keywords:
        skipped.add("seed")
    ...
    for phase_name in PHASES:
        if phase_name in skipped or state.get(phase_name) == "done":
            continue
        print(f"\n\n{'#'*60}")
