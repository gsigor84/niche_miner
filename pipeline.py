...
def plan_sequential_phases(args, state):
    """Return the remaining phases for this run in execution order."""
    skipped_phases = set()
    if args.skip_seed or args.keywords:
        skipped_phases.add("seed")
    if args.skip_scout:
        skipped_phases.add("scout")
    if args.skip_gap:
        skipped_phases.add("gap")

    return [
        phase_name
        for phase_name in PHASES
        if phase_name not in skipped_phases and state.get(phase_name) != "done"
    ]
...
def main():
    ...
    # Sequential pipeline. Plan from the requested phase order, not from the
    # pre-run state of earlier phases; each phase updates state as it runs.
    if args.keywords:
    ...
    phases_to_run = [(phase_name, phase_fns[phase_name])
                     for phase_name in plan_sequential_phases(args, state)]
