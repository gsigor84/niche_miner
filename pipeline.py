...
def parse_scout_subs(stdout):
    """Extract the --subs handoff printed by scout_subreddits.py."""
...
def run_phase_scout(args, run_id, state):
    ...
    discovered_subs = parse_scout_subs(result.stdout or "")
    if discovered_subs:
        state["scout_subs"] = discovered_subs
        print(f"[PHASE 2] Discovered subreddits: {','.join(discovered_subs)}")
    elif not args.subs:
        print("[ERROR] scout_subreddits found no usable subreddits; pass --subs or broaden keywords.")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    subs = args.subs or ",".join(state.get("scout_subs", []))
    if subs:
    ...
    raw_out = DATA / f"{args.run_id}_raw.jsonl"
    if not raw_out.exists() or raw_out.stat().st_size == 0:
        print(f"[ERROR] Fetch produced no raw output: {raw_out}")
        sys.exit(1)
    state["fetch"] = "done"
    save_run_state(run_id, state)
...
def parse_args():
    ...
    p.add_argument("--only_pain_points", action="store_true", help="Only fetch/normalize pain-point-ish posts")
    # gap phase
...
def main():
    ...
    # Sequential pipeline. Evaluate completion after each phase so a fresh
    # run does not stop after seed/scout because state was empty at startup.
    phases_to_run = [
        ("seed", run_phase_seed),
        ("scout", run_phase_scout),
        ("fetch", run_phase_fetch),
    ...
    for phase_name, phase_fn in phases_to_run:
        if phase_name in skipped:
            print(f"[SKIP] {phase_name} phase skipped by CLI")
            continue
