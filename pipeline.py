...
def scouted_subs_file(run_id):
    return RUNS / run_id / "subreddits.txt"

def load_scouted_subs(run_id):
    path = scouted_subs_file(run_id)
    if not path.exists():
        return []
...
def run_phase_scout(args, run_id, state):
    ...
    if args.subs:
        print("[SKIP] Using provided subreddits, skipping scout_subreddits")
        state["scout"] = "done"
        save_run_state(run_id, state)
        return
    if args.keywords:
    ...
    out_path = scouted_subs_file(run_id)
    cmd = [
    ...
    if result.returncode != 0:
        print(f"[FAIL] scout_subreddits failed with code {result.returncode}")
        sys.exit(result.returncode)
    if not load_scouted_subs(run_id):
        print("[FAIL] scout_subreddits produced no subreddits")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    subs = args.subs
    if not subs:
        scouted = load_scouted_subs(run_id)
        if scouted:
            subs = ",".join(scouted)
    if not subs:
    ...
    seen_path = DATA / f"{args.run_id}_seen_post_ids.txt"
    cmd = [
    ...
    if result.returncode != 0:
        print(f"[FAIL] rss_miner failed with code {result.returncode}")
        sys.exit(result.returncode)
    if not raw_path.exists() or raw_path.stat().st_size == 0:
        print(f"[FAIL] rss_miner produced no raw output: {raw_path}")
        sys.exit(1)
    state["fetch"] = "done"
    save_run_state(run_id, state)
...
def parse_args():
    ...
    p.add_argument("--niche", dest="topic", help="Alias for --topic")
...
def main():
    ...
    for phase_name in PHASES:
        if phase_name == "seed" and args.skip_seed:
            continue
