...
def parse_discovered_subs(output):
    """Extract the scout_subreddits --subs hint from stdout."""
    for line in output.splitlines():
        marker = "To use with rss_miner: --subs "
        if marker in line:
            subs = line.split(marker, 1)[1].strip()
            return subs or None
    return None
...
def run_phase_scout(args, run_id, state):
    ...
    discovered_subs = parse_discovered_subs(result.stdout or "")
...
def run_phase_fetch(args, run_id, state):
    ...
    subs = args.subs or state.get("subs") or args.niche_type
    cmd = [
        ...
        "--seen", f"runs/{args.run_id}/seen_post_ids.txt",
        "--include_comments",
        "--include_top",
        "--include_search",
        "--t", "month",
...
def main():
    ...
    # Sequential pipeline. Decide each phase after previous phases update state so
    # a fresh run can continue through fetch/normalize/gap in one invocation.
    if args.keywords:
    ...
    phase_fns = [
        ("seed", run_phase_seed, not args.skip_seed and state.get("seed") != "done"),
        ("scout", run_phase_scout, not args.skip_scout and state.get("scout") != "done"),
        ("fetch", run_phase_fetch, state.get("fetch") != "done"),
        ("normalize", run_phase_normalize, state.get("normalize") != "done"),
        ("gap", run_phase_gap, not args.skip_gap and state.get("gap") != "done"),
    ]

    ran_any = False
    for phase_name, phase_fn, should_run in phase_fns:
        if not should_run:
            continue
        if phase_name == "fetch" and state.get("scout") != "done" and not args.skip_scout:
            continue
