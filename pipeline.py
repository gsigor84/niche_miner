...
def run_phase_scout(args, run_id, state):
    ...
    if not discovered_subs:
        print("[ERROR] scout_subreddits did not discover any subreddits; refusing to fetch unrelated data.")
        sys.exit(1)
...
def run_phase_fetch(args, run_id, state):
    ...
    if args.keywords:
        cmd.extend(["--keywords", args.keywords, "--max_keywords", str(args.max_seeds)])
