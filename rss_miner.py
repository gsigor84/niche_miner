...
def parse_keywords(value: str, prefix: Optional[str] = None) -> List[str]:
    """Parse comma-separated keywords from the CLI."""
    keywords = []
...
def main():
    ...
    ap.add_argument("--seed_file", default=SEED_TOPICS_FILE, help="Seed keyword file")
    ...
    # Load keywords from CLI or seed file.
    if args.keywords:
    ...
    if args.include_search and not args.keywords:
        print("[ERROR] Search feeds require at least one keyword. Provide --keywords or a non-empty --seed_file.")
        sys.exit(1)
