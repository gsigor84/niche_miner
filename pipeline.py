...
def run_phase_scout(args, run_id, state):
    ...
    subs_match = re.search(r"To use with rss_miner:\s+--subs\s+([^\s]+)", result.stdout or "")
    if subs_match:
        state["subs"] = subs_match.group(1)
    elif not args.subs:
        print("[FAIL] scout_subreddits did not discover any subreddits")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    if not raw.exists() or raw.stat().st_size == 0:
        print(f"[FAIL] rss_miner produced no raw output: {raw}")
        sys.exit(1)
    state["fetch"] = "done"
    save_run_state(run_id, state)
...
def run_phase_normalize(args, run_id, state):
    ...
    if not normalized.exists() or normalized.stat().st_size == 0:
        print(f"[FAIL] normalize produced no output: {normalized}")
        sys.exit(1)
...
def run_phase_gap(args, run_id, state):
    ...
    if not gap_output.exists() or gap_output.stat().st_size == 0:
        print(f"[FAIL] gap_analysis produced no output: {gap_output}")
        sys.exit(1)
    state["gap"] = "done"
    save_run_state(run_id, state)
...
def parse_args():
    ...
    p.add_argument("--only_pain_points", action="store_true", help="Only keep pain-point posts during fetch")
    # gap phase
...
def main():
    ...
    # Sequential pipeline. Evaluate state after each phase so fresh runs continue
    # into downstream phases that are made possible by earlier successful phases.
    phases = [
        ("seed", run_phase_seed),
        ("scout", run_phase_scout),
        ("fetch", run_phase_fetch),
