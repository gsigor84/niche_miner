...
def plan_pipeline_phases(args, state):
    """Return the incomplete full-pipeline phases in execution order."""
    phase_plan = []
    if not args.keywords and not args.skip_seed:
        phase_plan.append(("seed", run_phase_seed))
    if not args.skip_scout:
        phase_plan.append(("scout", run_phase_scout))
    phase_plan.append(("fetch", run_phase_fetch))
    phase_plan.append(("normalize", run_phase_normalize))
    if not args.skip_gap:
        phase_plan.append(("gap", run_phase_gap))
    return [(name, fn) for name, fn in phase_plan if state.get(name) != "done"]
...
def main():
    ...
    # Sequential pipeline
    phases_to_run = plan_pipeline_phases(args, state)
