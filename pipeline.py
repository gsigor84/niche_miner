...
def main():
    ...
    # Sequential pipeline. Plan the ordered workflow up front, then decide whether
    # to run each phase after the previous phase has updated persisted state.
    phases_to_run = []
    if args.keywords:
    ...
    for phase_name, phase_fn in phases_to_run:
        if state.get(phase_name) == "done":
            print(f"[SKIP] Phase {phase_name} already complete")
            continue
        print(f"\n\n{'#'*60}")
