...
def run_fetch(args):
    ...
    output_existed = out_path.exists()
    saved_count = 0
    ...
    if saved_count == 0 and not output_existed and not out_path.exists():
        print("[ERROR] Fetch completed without creating any output records.", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    if args.include_search and not args.keywords:
        print("[ERROR] Search feeds require keywords. Provide --keywords or seed_topics.txt.", file=sys.stderr)
        sys.exit(1)
