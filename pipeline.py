...
def load_seed_keywords(max_seeds):
    ...
    if not seeds:
        print("[ERROR] seed_topics.txt contains no usable keywords.")
        sys.exit(1)
...
def run_phase_scout(args, run_id, state):
    ...
    if args.keywords:
        ...
        keywords = load_seed_keywords(args.max_seeds)
    ...
    cmd = ["python3", "scout_subreddits.py", keyword_arg, "--limit", str(args.max_seeds)]
    print(f"\n[PHASE 2] Keywords: {keyword_arg[:100]}...")
    ...
    discovered_subs = parse_scout_subreddits(result.stdout)
    if discovered_subs:
        state["subs"] = ",".join(discovered_subs)
        print(f"[PHASE 2] Discovered subreddits saved: {state['subs']}")
    elif not args.subs:
        print("[ERROR] No subreddits discovered and no --subs override was provided.")
        sys.exit(1)
...
def run_phase_fetch(args, run_id, state):
    ...
    subs = args.subs or state.get("subs")
    if not subs:
    ...
    if args.max_keywords:
        cmd.extend(["--max_keywords", str(args.max_keywords)])
    if args.only_pain_points:
        cmd.append("--only_pain_points")

    run(cmd, "PHASE 3: rss_miner")
    if not raw.exists() or raw.stat().st_size == 0:
        print(f"[ERROR] rss_miner produced no raw data at {raw}")
        sys.exit(1)
...
def parse_args():
    ...
    p.add_argument("--max_keywords", type=int, default=None, help="Max keywords to use during fetch")
    p.add_argument("--only_pain_points", action="store_true", help="Fetch only pain-point-ish posts")
    # gap phase
...
def main():
    ...
    # Sequential pipeline. Decide each phase after prior phases update state.
    ran_any = False
    if args.keywords:
    ...
    for phase_name, phase_fn in phase_fns:
        if phase_name == "seed" and (args.skip_seed or args.keywords):
            if state.get("seed") != "done":
                state["seed"] = "done"
                save_run_state(args.run_id, state)
            continue
        if phase_name == "scout" and args.skip_scout:
            if not args.subs and not state.get("subs"):
                print("[ERROR] --skip_scout requires --subs or saved scout subreddits.")
                sys.exit(1)
            if state.get("scout") != "done":
                state["scout"] = "done"
                save_run_state(args.run_id, state)
            continue
        if phase_name == "gap" and args.skip_gap:
            continue
