...
def main():
    ...
    if args.source == "google":
        generated_count = factory.harvest_google_taxonomy()
    elif args.source == "llm":
        if not args.topic:
            print("[ERROR] --topic is required for LLM source.")
            return 2
        generated_count = factory.brainstorm_llm(args.topic, count=args.count, model=args.model)

    if args.source in {"google", "llm"} and generated_count == 0:
        print(f"[ERROR] No seeds generated; leaving {factory.output_path} unchanged.")
        return 1

    if not factory.seeds:
        print(f"[ERROR] No seeds available; leaving {factory.output_path} unchanged.")
        return 1
