...
def extract_scout_subs(output):
    """Extract the machine-readable subreddit list printed by scout_subreddits."""
...
def parse_args():
    ...
    p.add_argument("--only_pain_points", action="store_true", help="Only fetch pain-point-like posts")
    # gap phase
...
def main():
    ...
    # Sequential pipeline. Evaluate state after each phase so a fresh run can
    # continue through downstream phases in the same process.
    if args.keywords and state.get("seed") != "done":
        state["seed"] = "done"
        save_run_state(args.run_id, state)

    phase_plan = [
        ("seed", run_phase_seed, lambda: args.skip_seed or bool(args.keywords)),
        ("scout", run_phase_scout, lambda: args.skip_scout),
        ("fetch", run_phase_fetch, lambda: False),
        ("normalize", run_phase_normalize, lambda: False),
        ("gap", run_phase_gap, lambda: args.skip_gap),
    ]

    ran_phase = False
    for phase_name, phase_fn, should_skip in phase_plan:
        if should_skip() or state.get(phase_name) == "done":
            continue
        print(f"\n\n{'#'*60}")
