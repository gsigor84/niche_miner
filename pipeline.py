...
def run_phase_scout(args, run_id, state):
    ...
    if not args.subs:
        for line in result.stdout.splitlines():
            ...
            if marker in line:
                state["scouted_subs"] = line.split(marker, 1)[1].strip()
                ensure_dir(RUNS / run_id)
                (RUNS / run_id / "subreddits.txt").write_text(state["scouted_subs"])
                break
        if not state.get("scouted_subs"):
            print("[ERROR] Scout did not discover subreddits; pass --subs to fetch explicitly.")
            sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    subs = args.subs or state.get("scouted_subs")
    if subs:
...
def main():
    ...
    if args.resume or args.phase:
        ...
        state_file = RUNS / args.run_id / "state.json"
        if state_file.exists() and not args.phase:
            print(f"[ERROR] Run state already exists for {args.run_id}. Use --resume or choose a new --run_id.")
            sys.exit(1)
        state = {}
