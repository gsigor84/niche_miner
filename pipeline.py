...
def parse_scout_subs(output):
    """Extract the scout's recommended subreddit list from its CLI output."""
    for line in output.splitlines():
        if "--subs" in line:
            subs = line.split("--subs", 1)[1].strip().split()[0]
            return ",".join(s.strip() for s in subs.split(",") if s.strip())
        match = re.search(r"Top discovered subreddits:\s*(.+)", line)
        if match:
            return ",".join(s.strip() for s in match.group(1).split(",") if s.strip())
    return None
...
def run_phase_scout(args, run_id, state):
    ...
    if result.returncode != 0:
        ...
        sys.exit(1)
    scout_subs = parse_scout_subs(result.stdout)
    if scout_subs:
        state["scout_subs"] = scout_subs
        print(f"[INFO] Scout discovered subreddits: {scout_subs}")
    state["scout"] = "done"
    save_run_state(run_id, state)

def run_phase_fetch(args, run_id, state):
    """Phase 3: Fetch Reddit data via rss_miner."""
    subs = args.subs or state.get("scout_subs") or args.niche_type
    cmd = [
...
def main():
    ...
    if not args.skip_seed and not args.keywords:
        phase_order.append("seed")
    if not args.skip_scout:
        phase_order.append("scout")
    phase_order.extend(["fetch", "normalize"])
    if not args.skip_gap:
        phase_order.append("gap")

    phases_to_run = [phase for phase in phase_order if state.get(phase) != "done"]
    ...
    for phase_name in phases_to_run:
        ...
        print(f"{'#'*60}")
        phase_fn[phase_name](args, args.run_id, state)
        time.sleep(0.5)
