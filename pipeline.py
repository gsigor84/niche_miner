...
def parse_scout_subs(output):
    """Extract the comma-separated subreddit list printed by scout_subreddits."""
...
def run_phase_scout(args, run_id, state):
    ...
    if discovered_subs:
        ...
        print("[ERROR] scout_subreddits did not discover any subreddits; pass --subs or broaden the keywords.")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def main():
    ...
    # Sequential pipeline. Evaluate state as each phase completes so a fresh run
    # continues through downstream phases in the same invocation.
    phase_order = [
        ("seed", run_phase_seed),
        ("scout", run_phase_scout),
        ("fetch", run_phase_fetch),
    ...
    skipped = {
        "seed": args.skip_seed,
        "scout": args.skip_scout,
        "gap": args.skip_gap,
    }
    ran_any = False

    for phase_name, phase_fn in phase_order:
        if skipped.get(phase_name, False) or state.get(phase_name) == "done":
            continue
        print(f"\n\n{'#'*60}")
