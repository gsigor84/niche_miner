...
def planned_phases(args, state):
    """Return remaining phases in dependency order for the current run."""
    phases = []
    if not args.skip_seed and state.get("seed") != "done":
        phases.append(("seed", run_phase_seed))
    if not args.skip_scout and state.get("scout") != "done":
        phases.append(("scout", run_phase_scout))
    if state.get("fetch") != "done":
        phases.append(("fetch", run_phase_fetch))
    if state.get("normalize") != "done":
        phases.append(("normalize", run_phase_normalize))
    if not args.skip_gap and state.get("gap") != "done":
        phases.append(("gap", run_phase_gap))
    return phases
...
def main():
    ...
    print(f"  Subs:       {args.subs or '(rss_miner defaults)'}")
    ...
    phases_to_run = planned_phases(args, state)
