...
def build_phase_plan(args, state):
    """Return the ordered phases that still need to run."""
    phases = []
    if not args.keywords and not args.skip_seed and state.get("seed") != "done":
        phases.append("seed")
...
def run_phase_scout(args, run_id, state):
    ...
    if scouted_subs and not args.subs:
        args.subs = scouted_subs
...
def main():
    ...
    phases_to_run = [(phase_name, phase_fn[phase_name]) for phase_name in build_phase_plan(args, state)]
