...
def parse_scout_subs(output):
    """Extract the comma-separated --subs value printed by scout_subreddits."""
...
def run_phase_scout(args, run_id, state):
    ...
    if discovered_subs:
        ...
        print("[ERROR] No subreddits discovered. Provide --subs or broader keywords.")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    subs = args.subs or ",".join(state.get("subs", []))
    if subs:
...
def should_run_phase(phase_name, args, state):
    if state.get(phase_name) == "done":
        return False
    if phase_name == "seed":
        return not args.skip_seed and not args.keywords
    if phase_name == "scout":
        return not args.skip_scout
    if phase_name == "fetch":
        return state.get("seed") == "done" or state.get("scout") == "done"
    if phase_name == "normalize":
        return state.get("fetch") == "done"
    if phase_name == "gap":
        return not args.skip_gap and state.get("normalize") == "done"
    return False
...
def main():
    ...
    # Sequential pipeline. Decide each phase after the previous phase updates
    # state so fresh full runs do not skip downstream work.
    phase_plan = [
        ("seed", run_phase_seed),
        ("scout", run_phase_scout),
        ("fetch", run_phase_fetch),
    ...
    ran_phase = False
    for phase_name, phase_fn in phase_plan:
        if not should_run_phase(phase_name, args, state):
            continue
        ran_phase = True
        print(f"\n\n{'#'*60}")
