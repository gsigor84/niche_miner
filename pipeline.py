...
def build_phase_plan(args, state):
    """Return unfinished phases in execution order for a full pipeline run."""
    phase_fn = {
        "seed": run_phase_seed,
    ...
    if not args.keywords and not args.skip_seed:
        phase_names.append("seed")
    if not args.skip_scout:
        phase_names.append("scout")
    phase_names.extend(["fetch", "normalize"])
    if not args.skip_gap:
        phase_names.append("gap")
    return [(name, phase_fn[name]) for name in phase_names if state.get(name) != "done"]
