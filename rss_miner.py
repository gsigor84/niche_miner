...
def run_fetch(args):
    ...
    if saved_count == 0 and (not out_path.exists() or out_path.stat().st_size == 0):
        print("[ERROR] No posts saved; leaving downstream phases without input.", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    if args.keywords:
        ...
        args.keywords = [f"{args.prefix} {k}" if args.prefix else k for k in raw_keywords]
    else:
    ...
    if args.include_search and not args.keywords:
        print("[ERROR] Search feeds requested but no keywords were provided or loaded.", file=sys.stderr)
        sys.exit(1)
