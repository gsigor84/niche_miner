...
def parse_keywords(value: Optional[str], prefix: Optional[str] = None) -> List[str]:
    ...
    if prefix:
        keywords = [f"{prefix} {kw}" for kw in keywords]
    return keywords
...
def run_fetch(args):
    ...
    if not feeds:
        print("[ERROR] No feeds generated; nothing to fetch.")
        raise SystemExit(1)
    ...
    if not out_path.exists() or out_path.stat().st_size == 0:
        print(f"[ERROR] No posts were saved to {out_path}.")
        raise SystemExit(1)
...
def main():
    ...
    ap.add_argument("--max_keywords", type=int, default=None, help="Max keywords to use")
    ...
    # Load keywords from explicit CLI input or seed_topics.txt.
    if args.keywords:
