...
def run_phase_scout(args, run_id, state):
    ...
    if args.subs:
        print(f"[PHASE 2] Explicit subreddits provided: {args.subs}")
    ...
    match = re.search(r"To use with rss_miner:\s+--subs\s+([^\s]+)", stdout)
    if not match:
        print("[ERROR] scout_subreddits did not discover any subreddits.")
        sys.exit(1)

    state["subs"] = match.group(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def main():
    ...
    # Sequential pipeline. Plan by phase order, not the pre-run state; each
    # phase updates state as it succeeds, and completed phases are skipped.
    phase_plan = [
    ...
    phases_to_run = [
        (name, fn)
        for name, fn, should_skip in phase_plan
        if not should_skip and state.get(name) != "done"
    ]
