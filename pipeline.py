...
def parse_scout_subs(output):
    """Extract the --subs value printed by scout_subreddits.py."""
    match = re.search(r"To use with rss_miner:\s+--subs\s+([^\s]+)", output or "")
    return match.group(1) if match else None

def selected_subs(args, state):
    """Choose explicit or scouted subreddits for rss_miner."""
    return args.subs or state.get("scout_subs")
...
def run_phase_scout(args, run_id, state):
    ...
    scouted_subs = parse_scout_subs(result.stdout)
    if scouted_subs:
        state["scout_subs"] = scouted_subs
    elif not args.subs:
        print("[FAIL] scout_subreddits did not discover any subreddits and --subs was not provided")
        sys.exit(1)
    state["scout"] = "done"
    save_run_state(run_id, state)
...
def run_phase_fetch(args, run_id, state):
    ...
    cmd = [
        ...
        "--out", f"data/{run_id}_raw.jsonl",
        "--seen", f"data/{run_id}_seen_post_ids.txt",
        "--include_comments",
        "--include_top",
        "--include_search",
        "--t", "month",
...
def run_sequential_pipeline(args, state):
    """Run each incomplete phase, evaluating state after every phase."""
    phases_run = []
    ...
    if not args.skip_seed and state.get("seed") != "done":
        run_phase_seed(args, args.run_id, state)
        phases_run.append("seed")
    if not args.skip_scout and state.get("scout") != "done":
        run_phase_scout(args, args.run_id, state)
        phases_run.append("scout")
    if state.get("fetch") != "done":
        run_phase_fetch(args, args.run_id, state)
        phases_run.append("fetch")
    if state.get("normalize") != "done":
        run_phase_normalize(args, args.run_id, state)
        phases_run.append("normalize")
    if not args.skip_gap and state.get("gap") != "done":
        run_phase_gap(args, args.run_id, state)
        phases_run.append("gap")
    return phases_run
...
def parse_args():
    ...
    p.add_argument("--only_pain_points", action="store_true", help="Only fetch pain-point-ish posts")
    return p.parse_args()
...
def main():
    ...
    phases_to_run = run_sequential_pipeline(args, state)
